...
def inference(images):
  ...
  with tf.name_scope('conv1') as scope:
    kernel = tf.Variable(tf.truncated_normal([1, 17, 1, 96], dtype=tf.float32,
  ...
  with tf.name_scope('conv2') as scope:
    kernel = tf.Variable(tf.truncated_normal([1, 13, 96, 256], dtype=tf.float32,
                                             stddev=1e-1), name='weights')
  ...
  with tf.name_scope('conv3') as scope:
    kernel = tf.Variable(tf.truncated_normal([1, 9, 256, 384],
                                             dtype=tf.float32,
                                             stddev=1e-1), name='weights')
  ...
  with tf.name_scope('conv4') as scope:
    kernel = tf.Variable(tf.truncated_normal([1, 9, 384, 384],
                                             dtype=tf.float32,
                                             stddev=1e-1), name='weights')
  ...
  with tf.name_scope('conv5') as scope:
    kernel = tf.Variable(tf.truncated_normal([1, 9, 384, 256],
                                             dtype=tf.float32,
                                             stddev=1e-1), name='weights')
  ...
  #平铺
  pool5=tf.reshape(pool5,[-1,256*73*1])
  # 全连接
  with tf.name_scope('full_layer'):
    weights = tf.Variable(
        tf.truncated_normal([256*73*1, 1000],
                            stddev=0.1),
        name='weights')
