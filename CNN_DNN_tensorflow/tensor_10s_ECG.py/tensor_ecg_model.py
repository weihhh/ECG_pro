...
def inference(images, conv1_chanel, conv2_chanel,full_chanel):
  ...
  #平铺
  pool2=tf.reshape(pool2,[-1,conv2_chanel*99*1])
  # 全连接
  with tf.name_scope('full_layer'):
    weights = tf.Variable(
        tf.truncated_normal([conv2_chanel*99*1, full_chanel],
                            stddev=0.1),
        name='weights')
...
def loss(logits, labels):
  ...
  #     logits=logits[:,i:i+6], labels=labels[:,j], name='xentropy')
...
def accuracy(logits,labels):
  ...
  correct_int=tf.cast(correct_bool, tf.int32)#对应标签反应是否预测正确
  correct=tf.reduce_sum(correct_int)
  precision = correct /correct_int.shape[0]
  return precision,correct_int,labels
