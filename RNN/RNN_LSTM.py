...
def RNN(x, weights, biases):
    ...
    # Get lstm cell output，outputs——time_steps*batchsize*input
    outputs, states = rnn.static_rnn(lstm_cell, x, dtype=tf.float32)

    # Linear activation, using rnn inner loop last output，相当于全连接层
    #最后outputs取的最后一个，因为这里只是需要一个分类输出
